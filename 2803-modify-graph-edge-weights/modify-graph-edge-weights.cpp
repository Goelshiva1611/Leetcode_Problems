...
public:
    ...
    ll DijkstraAlgo(vector<vector<int>>& edges, int n, int src, int dest) {
        ...
        vector<ll> result(
            n,
            INT_MAX); // result[i] = shortest path distance of src to ith node
    ...
    vector<vector<int>> modifiedGraphEdges(int n, vector<vector<int>>& edges,
        ...
        for (vector<int>& edge : edges) {
            if (edge[2] == -1) {
                ...
                edge[2] = (matchedTarget == true)
                              ? LARGE_VALUE
                              : 1; // assign lowest number i.e. 1

                if (matchedTarget != true) {
                    ll newShortestDist =
                        DijkstraAlgo(edges, n, source, destination);
